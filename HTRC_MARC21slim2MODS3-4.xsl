...
Revision HT1.22  - Added note when <genre authority = "marcgt">biography</genre> is added
					in response to MARC 008/34 cf 2013/11/14 
Revision HT1.21  - Undid revision HT1.03, because the handling was found elsewhere.
Revision HT1.20  - Added handling of controlField008-28 cf 2013/11/14
Revision HT1.10  - Added handling of datafield 974 by adding templates cf 2013/11/14
					createLocationFrom974
					createAccessConditionFrom974
Revision HT1.04  - Added handling of controlField008-33='m' by adding genre of mixed form
					 and handling of controlField008-33='0' by adding genre of not fiction. cf 2013/11/06
Revision HT1.03  - Added handling of controlField008-24-27='j' by adding genre of patent. cf 2013/11/14
Revision HT1.02  - Added handling of controlField008-33='p' by adding genre of poetry. cf 2013/11/06
Revision HT1.01  - Retyped conditional statements for <xsl:when test="$controlField008-33='l'"> cf 2013/11/01
Revision HT1.00  - Changed Filename - cf 2013/10/29
...
-->
		...
		<xsl:if test="$typeOf008='BK'">
			...
			<xsl:if test="$controlField008-34='a' or $controlField008-34='b' or $controlField008-34='c' or $controlField008-34='d'">
				...
				<xsl:choose>
					<xsl:when test="$controlField008-34='a'">
						<note>biography -- autobiography</note>
					</xsl:when>
					<xsl:when test="$controlField008-34='b'">
						<note>biography -- individual biography</note>
					</xsl:when>
					<xsl:when test="$controlField008-34='c'">
						<note>biography -- collective biography</note>
					</xsl:when>
					<xsl:when test="$controlField008-34='d'">
						<note>biography -- contains biographical information</note>
					</xsl:when>
				</xsl:choose>
			</xsl:if>
			...
			<xsl:choose>
				...
				<xsl:when test="$controlField008-33='0'">
					<genre authority="marcgt">not fiction</genre>
				</xsl:when>
				<xsl:when test="$controlField008-33='m'">
					<genre authority="marcgt">mixed form</genre>
				</xsl:when>
			</xsl:choose>
			<xsl:variable name="controlField008-28" select="substring($controlField008,29,1)"/>
			<xsl:if test="contains('acfilmosuz', $controlField008-28)">
				<genre authority="marcgt">government publication</genre>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="$controlField008-28='a'">
					<note>government publication -- autonomous or semi-autonomous component</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='c'">
					<note>government publication -- multilocal</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='f'">
					<note>government publication -- federal/national</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='i'">
					<note>government publication -- international intergovernmental</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='l'">
					<note>government publication -- local</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='m'">
					<note>government publication -- multistate</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='o'">
					<note>government publication -- level undetermined</note>
				</xsl:when>
				<xsl:when test="$controlField008-28='s'">
					<note>government publication -- state, provincial, territorial, dependent, etc.</note>
				</xsl:when>
			</xsl:choose>
		</xsl:if>
		...
		<xsl:for-each select="marc:datafield[@tag=974]">
			<xsl:call-template name="createAccessConditionFrom974"/>
		</xsl:for-each>
...
-->
		...
		<recordInfo>
			...
			<recordOrigin>Converted from MARCXML to MODS version 3.4 using HTRC_MARC21slim2MODS3-4.xsl (Revision HT1.25 2014/10/10)</recordOrigin>
...
-->
	...
	<xsl:template name="createAccessConditionFrom974">
		<accessCondition>
			<xsl:call-template name="subfieldSelect">
				<xsl:with-param name="codes">r</xsl:with-param>
			</xsl:call-template>			
		</accessCondition>
	</xsl:template>
	...
	<xsl:template name="createLocationFrom974">
		<location>
			...
			<xsl:if test="marc:subfield[@code='u'] or marc:subfield[@code='z']">
				<holdingSimple>
					<copyInformation>
						<xsl:if test="marc:subfield[@code='u']">
							<electronicLocator>
